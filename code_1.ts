...
const SocioTitularForm: React.FC = () => {
  ...
  const [searchType, setSearchType] = useState<'db' | 'api'>('db'); // 'db' or 'api'
    ...
    try {
      ...
      if (searchType === 'db') {
        ...
        if (!apiResponse.success) {
          ...
          const dniFromApi = apiResponse.data.dni || apiResponse.data.document_number;
          const formattedDni = dniFromApi && dniFromApi.length === 8 ? dniFromApi : dniFromApi;
          ...
          data = {
            ...
            dni: formattedDni, // Use formatted DNI
          };
        }
      }
