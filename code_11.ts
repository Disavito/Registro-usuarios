    ...
    try {
      ...
      if (searchType === 'db') {
        ...
        if (!apiResponse.success) {
          ...
          // Use apiResponse.data.number as the DNI
          let dniFromApi = apiResponse.data.number;

          // Ensure dniFromApi is a string
          dniFromApi = String(dniFromApi);

          console.log('DNI from API:', dniFromApi); // Debugging line

          // Validate DNI from API
          if (!/^\d{8}$/.test(dniFromApi)) {
            console.log('Invalid DNI from API:', dniFromApi); // Debugging line
