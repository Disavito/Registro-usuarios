...
const initialFormData: SocioTitularRegistration = {
  ...
  apellidoPaterno: '',
  apellidoMaterno: '',
  edad: '', // Cambiado de 0 a cadena vacía para que el placeholder sea visible
  dni: '',
  direccionDNI: '',
  distritoDNI: '',
  provinciaDNI: '',
...
const SocioTitularForm: React.FC = () => {
  ...
  const handleChange = (e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    setFormData(prev => ({
      ...prev,
      [name]: type === 'number' ? (value === '' ? '' : parseInt(value, 10) || 0) : value, // Manejar cadena vacía para campos numéricos
    }));
  ...
  const validateForm = () => {
    ...
    // Validar edad solo si no está vacía
    if (formData.edad === '') {
    ...
    if (!formData.localidad) newErrors.localidad = 'Localidad es requerida.'; // Localidad ahora es obligatoria
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      let response;
      // Asegurarse de que la edad se envíe como número o null, no como cadena vacía
      const dataToSubmit = {
        ...formData,
